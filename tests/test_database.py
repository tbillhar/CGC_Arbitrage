...
def test_app_settings_round_trip(tmp_path: Path) -> None:
    ...
    assert settings["fixed_order_fee"] == "0.4"
...
def test_app_settings_update_existing_values(tmp_path: Path) -> None:
    ...
    db.save_app_settings(AppSettings(0.10, 0.03, 0.40, 18.0, 0.20))
    db.save_app_settings(AppSettings(0.12, 0.04, 0.30, 20.0, 0.15))
    ...
    assert settings["fixed_order_fee"] == "0.3"
